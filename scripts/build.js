...
function extractKeyedAttributes(html, key, attribute) {
  ...
  const pattern = new RegExp(
    `<[\\w:-]+\\b(?=[^>]*\\b${keyAttribute}=(['"])([^'"]+)\\1)(?=[^>]*\\b${attribute}=(['"])(.*?)\\3)[^>]*>`,
    "gi",
  );
  let match;
...
function mapTemplateValues(source, translations) {
  return Object.fromEntries(
    Object.entries(translations).map(([key, value]) => {
      if (!(key in source))
        throw new Error(`Missing template source for translation key: ${key}`);
...
const locales = Object.fromEntries(
  languages.map((language) => [
    language,
    readJson(path.join(root, "locales", `${language}.json`)),
  ]),
);
for (const language of languages) {
  for (const key of [
    ...
    if (locales[language].home[key] === undefined)
      throw new Error(`Missing home.${key} in locales/${language}.json`);
  }
  for (const key of [
    "translations",
    "ariaTranslations",
    ...
    if (locales[language].caseStudy[key] === undefined)
      throw new Error(`Missing caseStudy.${key} in locales/${language}.json`);
  }
}
...
for (const page of ["index.html", "how-we-work.html"]) {
  ...
  const caseStudyAria = isHome
    ? null
    : extractKeyedAttributes(template, "aria", "aria-label");
  const caseStudyAlt = isHome
    ? null
    : extractKeyedAttributes(template, "alt", "alt");
  ...
  for (const language of languages) {
    ...
    const metadata = isHome
      ? locale.home.metaContent
      : locale.caseStudy.metaTranslations;
