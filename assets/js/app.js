(() => {
  ...
  const profileCards = [
    ...document.querySelectorAll(".person-card[data-profile]"),
  ];
  ...
  function syncThemeColour() {
    ...
    themeColourMeta.setAttribute(
      "content",
      root.dataset.theme === "dark" ? "#11131a" : "#ffffff",
    );
  }
  ...
  function focusLanguageOption(direction = 1) {
    const currentIndex = languageOptionButtons.findIndex(
      (option) => option === document.activeElement,
    );
    const selectedIndex = languageOptionButtons.findIndex((option) =>
      option.classList.contains("selected"),
    );
    const baseIndex = currentIndex >= 0 ? currentIndex : selectedIndex;
    const nextIndex =
      (baseIndex + direction + languageOptionButtons.length) %
  ...
  languageTrigger.addEventListener("click", () => {
    ...
    if (willOpen) {
      const selected = languageOptionButtons.find((option) =>
        option.classList.contains("selected"),
      );
      selected?.focus();
    }
